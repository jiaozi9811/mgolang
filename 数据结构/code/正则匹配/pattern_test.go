...
import (
	"fmt"
	"gorequest/boilr/pkg/util/validate/pattern"
	"testing"
)
...
//			t.Errorf("pattern.UnixPath.Math.MatchString(%q) expected to be %v",test.String,test.Valid)
//		}
//	}
//}
//
...
func TestEmailPattern(t *testing.T)  {
	tests:=[]struct{
		...
		{"@", false},
		{"sdfsddg@fgasf", false},
		{"sdfsd@dsfgsdf.com",true},
		{"sdf@",false},
		{"@dgsdfsd.com",false},
	}

	for _,test:=range tests{
		if ok:=pattern.Email.MatchString(test.String);ok!=test.Valid{
			t.Errorf("pattern.Email.MatchString(%q) expected to be %v", test.String, test.Valid)
		}
	}
}
